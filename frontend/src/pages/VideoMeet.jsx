        ...
        <div>
            {askForUsername === true ? (
                <div className={styles.lobbyContainer}>
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            '&.Mui-disabled': { bgcolor: 'rgba(79,142,247,0.3)', color: 'rgba(255,255,255,0.4)' },
                            borderRadius: '8px',
                            padding: '10px 28px',
                            fontWeight: 600,
                            color: 'white',
                ...
                <div className={styles.meetVideoContainer}>
                    {/* Hidden video fed to MediaPipe gesture detection */}
                    <video
                        ref={gestureVideoRef}
                        style={{ display: 'none' }}
                    ...
                    <div className={styles.copyLinkBtn} onClick={copyMeetingLink}>
                    ...
                    <div className={styles.conferenceView}>
                        {videos.map((v) => (
                            <div key={v.socketId} className={`${styles.videoWrapper} ${speakingUsers.has(v.socketId) ? styles.speaking : ''}`}>
                                <video
                                    key={v.socketId + (v.stream?.id || '')}
                                    ref={ref => {
                                    ...
                                    autoPlay playsInline muted={false} crossOrigin="anonymous"
                                />
                                <div className={styles.videoNameTag}>
                                    {socketUsernames[v.socketId] || 'Guest'}
                                </div>
                                {remoteCaptions[v.socketId] && (
                    ...
                    <video
                        className={`${styles.meetUserVideo} ${speakingUsers.has(socketIdRef.current) ? styles.speakingLocal : ''}`}
                        ref={localVideoref}
                        autoPlay
                        muted
                    />

                    {/* Corner floating reactions (button-triggered or received via socket) */}
                    ...
                    {/* Floating emoji particles from gesture effects */}
                    <div className={styles.floatingEmojiContainer}>
                        {floatingEmojis.map(e => (
                            <span
                                key={e.id}
                                className={`${styles.floatingEmoji} ${
                                    BALLOON_EMOJIS.has(e.emoji) ? styles.balloon :
                                    HEART_EMOJIS.has(e.emoji)   ? styles.heartEmoji :
                                    FIRE_EMOJIS.has(e.emoji)    ? styles.fireEmoji : ''
                                }`}
                                style={{
                                    left: `${e.x}%`,
                                    fontSize: `${e.size}rem`,
                    ...
                    {/* Full-screen gesture overlay */}
                    {gestureReaction && (
                        <div key={gestureReaction.id} className={styles.gestureOverlay}>
                            <span className={styles.gestureEmoji}>{gestureReaction.emoji}</span>
                        </div>
                    )}
                                ...
                                <div className={styles.chattingDisplay}>
                                    {messages.length !== 0 ? messages.map((item, index) => (
                                        <div className={styles.messageItem} key={index}>
                                            <p className={styles.messageSender}><strong>{item.sender}</strong></p>
                    ...
                    {translationPanelOpen && (
                        <div className={styles.translationPanel}>
                            <div className={styles.translationPanelHeader}>
                                <span>🌐 Live Translation</span>
                                <button className={styles.chatCloseBtn} onClick={() => setTranslationPanelOpen(false)}>✕</button>
                            </div>

                            <div className={styles.translationSettings}>
                                <div className={styles.translationLangRow}>
                                    <span className={styles.translationLangLabel}>🎤 I speak:</span>
                                    <select value={myLang} onChange={e => setMyLang(e.target.value)} className={styles.translationSelect}>
                                        {TRANSLATION_LANGS.map(l => (
                                            <option key={l.speak} value={l.speak}>{l.flag} {l.label}</option>
                                        ))}
                                    </select>
                                </div>
                                <div className={styles.translationLangRow}>
                                    <span className={styles.translationLangLabel}>👁️ Show in:</span>
                                    <select value={showInLang} onChange={e => setShowInLang(e.target.value)} className={styles.translationSelect}>
                                        {TRANSLATION_LANGS.map(l => (
                                            <option key={l.translate} value={l.translate}>{l.flag} {l.label}</option>
                                        ))}
                                    </select>
                                </div>
                                <button
                                    className={`${styles.translationStartBtn} ${translationOn ? styles.translationStartBtnActive : ''}`}
                                    onClick={() => translationOn ? stopTranslation() : startTranslation()}
                                >
                                    {translationOn ? '⏹ Stop Translation' : '▶ Start Translation'}
                                </button>
                            </div>

                            <div className={styles.translationMessages}>
                                {transcriptMessages.length === 0 ? (
                                    <div className={styles.translationEmpty}>
                                        <p>No transcriptions yet.</p>
                                        <p>Start translation and speak to see live transcripts here.</p>
                                    </div>
                                ) : (
                                    transcriptMessages.map(msg => (
                                        <div key={msg.id} className={styles.transcriptMessage}>
                                            <div className={styles.transcriptMeta}>
                                                <span className={styles.transcriptSender}>{msg.sender}</span>
                                                <span className={styles.transcriptTime}>{msg.timestamp}</span>
                                            </div>
                            ...
                            {transcriptMessages.length > 0 && (
                                <button className={styles.translationClearBtn} onClick={() => setTranscriptMessages([])}>
                                    Clear Transcript
                                </button>
                            )}
                        </div>
                    ...
                    {showReactionPicker && (
                        <div className={styles.reactionPickerPanel}>
                            <div className={styles.confettiToggleRow}>
                                <span className={styles.confettiLabel}>
                                    {confettiEnabled ? '✨ Effects ON' : '✨ Effects OFF'}
                                </span>
                                <div
                                    className={`${styles.toggleSwitch} ${confettiEnabled ? styles.toggleOn : ''}`}
                            ...
                            <div className={styles.emojiGrid}>
                                {[
                                    { emoji: '👋', effect: 'wave',       label: 'Wave' },
                                    { emoji: '🎉', effect: 'confetti',   label: 'Party' },
                                    { emoji: '👍', effect: 'confetti',   label: 'Like' },
                                    { emoji: '❤️', effect: 'hearts',     label: 'Love' },
                                    { emoji: '😂', effect: 'confetti',   label: 'Haha' },
                                    { emoji: '🔥', effect: 'fire',       label: 'Fire' },
                                    { emoji: '🎈', effect: 'balloons',   label: 'Balloon' },
                                    { emoji: '🤙', effect: 'celebrate',  label: 'Hang loose' },
                                    { emoji: '💥', effect: 'smashfire',  label: 'Boom' },
                                    { emoji: '🫰', effect: 'realhearts', label: 'Heart' },
                                ].map(r => (
                                    <button
                                        key={r.emoji}
                                        className={styles.emojiPickerBtn}
                                        onClick={() => sendReaction(r.emoji, r.effect)}
                                        title={r.label}
                                    >
                                        <span className={styles.emojiIcon}>{r.emoji}</span>
                                        <span className={styles.emojiPickerLabel}>{r.label}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className={styles.buttonContainers}>
                        ...
                        <IconButton
                            ...
                            style={{ color: showReactionPicker ? "#ffd93d" : "white", fontSize: "1.4rem" }}
                            title="Reactions"
                        >
                            😊
                        </IconButton>
                        <IconButton
                            onClick={() => translationOn ? stopTranslation() : startTranslation()}
                            style={{ color: translationOn ? "#4ade80" : "white" }}
                            title={translationOn ? "Stop Translation" : "Start Live Translation (Hindi→English)"}
                        >
                            <span style={{ fontSize: '0.8rem' }}>🌐</span>
                        </IconButton>
                        <IconButton
                            onClick={() => setTranslationPanelOpen(prev => !prev)}
                            style={{ color: translationPanelOpen ? "#4ade80" : "white" }}
